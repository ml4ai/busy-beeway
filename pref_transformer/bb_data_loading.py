...
def load_attempt_data(lvl,attempt,path=None,skip=0,control=1):
    if path == None:
    ...
    if control != 2 and control != 3:
        try: 
            ...
            raise FileNotFoundError(f"Could not find player data for level {lvl}, attempt {attempt}. Check that directory {path} exists!")
        ...
        for i in range(9,obs_end + 1):
            ...
            try:
                ...
                raise FileNotFoundError(f"Could not find data for entity (obstacle) {i} for level {lvl}, attempt {attempt}!")
        ...
        try:
            ...
            raise FileNotFoundError(f"Could not find data for goal-goal for level {lvl}, attempt {attempt}!")
        ...
        if g_df.shape[0] > 0:
            ...
            for i in range(g_df.shape[0]):
                ...
                p_pos = p_pos[(p_pos.index % (skip + 1) == 0) | (p_pos.index == (p_pos.shape[0] - 1))]
                p_pos = p_pos.reset_index(drop=True)
                p_pos["t"] = p_pos.index
                ...
                try:
                    ...
                    raise FileNotFoundError(f"Could not find data for entity (goal) {k} for level {lvl}, attempt {attempt}!")
                c_g = (c_g_df["posX"].values[0],c_g_df["posY"].values[0])
                D.append({"player": p_pos, "obstacles": o_pos, "reached_goal": True, "goal": c_g})
                
            if g_df.shape[0] < 7:
                p_pos = p_df.iloc[(g_df.iloc[g_df.shape[0]-1,1] - 1):p_df.shape[0],]
                p_pos = p_pos.reset_index(drop=True)
                p_pos = p_pos[(p_pos.index % (skip + 1) == 0) | (p_pos.index == (p_pos.shape[0] - 1))]
                p_pos = p_pos.reset_index(drop=True)
                p_pos["t"] = p_pos.index
                ...
                O_list = []
                for o_id,o_df in enumerate(o_dfs):
                    O = o_df.iloc[(g_df.iloc[g_df.shape[0]-1,1] - 1):p_df.shape[0],]
                    O = O.reset_index(drop=True)
                    O = O[(O.index % (skip + 1) == 0) | (O.index == (O.shape[0] - 1))]
                    O = O.reset_index(drop=True)
                    O["t"] = O.index
                ...
                try:
                    ...
                    raise FileNotFoundError(f"Could not find data for entity (goal) {k} for level {lvl}, attempt {attempt}!")
                c_g = (c_g_df["posX"].values[0],c_g_df["posY"].values[0])
                D.append({"player": p_pos, "obstacles": o_pos, "reached_goal": False, "goal": c_g})

        else:
            p_df = p_df[(p_df.index % (skip + 1) == 0) | (p_df.index == (p_df.shape[0] - 1))]
            p_df = p_df.reset_index(drop=True)
            p_df["t"] = p_df.index
                
            O_list = []
            for o_id,o_df in enumerate(o_dfs):
                O = o_df[(o_df.index % (skip + 1) == 0) | (o_df.index == (o_df.shape[0] - 1))]
                O = O.reset_index(drop=True)
                O["t"] = O.index
                O["id"] = o_id
                O_list.append(O)
            ...
            try:
                ...
                raise FileNotFoundError(f"Could not find data for entity (goal) 2 for level {lvl}, attempt {attempt}!")
            c_g = (c_g_df["posX"].values[0],c_g_df["posY"].values[0])
            D.append({"player": p_pos, "obstacles": o_pos, "reached_goal": False, "goal": c_g})
        ...
        if control == 2:
            ...
            if g_df.shape[0] > 0:
                ...
                for i in range(g_df.shape[0]):
                    ...
                    if h_id:
                        h_control = [list(map(itemgetter(1), g)) for k, g in groupby(enumerate(h_id), lambda x: x[0]-x[1])]
                        for k in h_control:
                ...
                if g_df.shape[0] < 7:
                    ...
                    p_g_pos = p_df.iloc[(g_df.iloc[g_df.shape[0]-1,1] - 1):p_df.shape[0],]
                    p_g_pos = p_g_pos.reset_index(drop=True)
                    h_id = p_g_pos[p_g_pos["userControl"] == True].index.to_list()
                    if h_id:
                        h_control = [list(map(itemgetter(1), g)) for k, g in groupby(enumerate(h_id), lambda x: x[0]-x[1])]
