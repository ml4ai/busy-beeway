...
source("~/busy-beeway/planners/lmdp/mct_data.R")

compute_log_rl <- function(dat,b_0,b_1,b_2,lambda) {
  vf <- create_vf_mct(b_0,b_1,b_2)
  rl <- 0
  n_sum <- 0 
  trans <- dat[[1]]
 ...
 (1/n_sum)*(rl + lambda*(b_0^2+b_1^2+b_2^2))
}
...
compute_grad_hess <- function(dat,b_0,b_1,b_2,lambda) {
  vf <- create_vf_mct(b_0,b_1,b_2)
  b_0_vf <- create_vf_mct(1,0,0)
  b_1_vf <- create_vf_mct(0,1,0)
  b_2_vf <- create_vf_mct(0,0,1)
  grad <- c(0,0,0)
  hess <- matrix(0,3,3)
  n_sum <- 0
