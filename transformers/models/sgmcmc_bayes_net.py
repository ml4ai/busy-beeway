...
from transformers.training.adaptive_sghmc import AdaptiveSGHMC
from transformers.training.sghmc import SGHMC
...
class BayesNet:
    ...
    def network_weights(self):
        ...
        params = nnx.state(self.net, nnx.Param)
        if self.weights_format == "tuple":
            return tuple(
                jnp.asarray(parameter) for parameter in jax.tree.flatten(params)[0]
            )
    ...
    def network_weights(self, weights):
        ...
        if self.weights_format == "tuple":
            params = nnx.variables(self.net, nnx.Param)
            for parameter, sample in zip(jax.tree.flatten(params)[0], weights):
                parameter.value = sample

        elif self.weights_format == "state_dict":
            nnx.update(self.net, weights)
    ...
    def _initialize_sampler(
        ...
        if self.sampling_method == "adaptive_sghmc":
            ...
            self.sampler = AdaptiveSGHMC(
                jax.tree.flatten(params)[0], **self.sampler_params
            )
        elif self.sampling_method == "sghmc":
            self.sampler = SGHMC(jax.tree.flatten(params)[0], **self.sampler_params)
