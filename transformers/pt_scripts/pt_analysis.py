...
def main(argv):
    ...
    parser.add_argument(
        ...
        "--output_dir",
        type=str,
        default="~/busy-beeway/transformers/dist_m_results",
    ...
    with h5py.File(data) as f:
        ...
        for i in range(t_samples):
            r_idx = rng.choice(f["states"].shape[0])
            sts = f["states"][r_idx, ...].reshape(1, 100, 26)
            acts = f["actions"][r_idx, ...].reshape(1, 100, 2)
            ...
            for r_model in tqdm(
                ...
                for j in range(am_sum):
                    ...
                    ts_samps = np.repeat(
                        ts[:, : (j + 1)].reshape(1, -1), n_samps, axis=0
                    )
                    am_samps = np.repeat(
                        am[:, : (j + 1)].reshape(1, -1), n_samps, axis=0
                    )
                    rwd_samps, _ = r_model(
        ...
        with h5py.File(f"{output_dir}/mean_dist_matrices.hdf5", "a") as g:
        ...
        plt.savefig(f"{output_dir}/mean_dist_matrices.png")
    sys.exit(0)
