...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Runs PT Analysis with Goal Only Trajectories",
    ...
    parser.add_argument(
        ...
        "--output_dir",
        type=str,
        default="~/busy-beeway/transformers/dist_m_goal_only_results",
    ...
    for i in range(t_samples):
        e_sts, e_acts, _ = bb_record_only_goal(
    ...
    with h5py.File(f"{output_dir}/mean_dist_matrices_goal_only.hdf5", "a") as g:
    ...
    plt.savefig(f"{output_dir}/mean_dist_matrices_goal_only.png")
    sys.exit(0)
